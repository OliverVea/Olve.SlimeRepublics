...
COPY src/Olve.SlimeRepublics/Olve.SlimeRepublics.csproj src/Olve.SlimeRepublics/
RUN dotnet restore src/Olve.SlimeRepublics -r linux-x64

COPY src/Olve.SlimeRepublics/ src/Olve.SlimeRepublics/
RUN dotnet publish src/Olve.SlimeRepublics -c Release -r linux-x64 -o /app
...
ENTRYPOINT ["./Olve.SlimeRepublics"]
