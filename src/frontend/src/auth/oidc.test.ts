...
describe("oidc", () => {
  ...
  it("restores a session from a persisted refresh token and exposes the user", async () => {
    localStorage.setItem("olve-slimerepublics.oidc.refresh", "rt-1");
    mockFetch([
      {
        body: {
          access_token: "at-1",
  ...
  it("clears the session when the refresh token is rejected", async () => {
    localStorage.setItem("olve-slimerepublics.oidc.refresh", "rt-expired");
    mockFetch([{ status: 400, body: { error: "invalid_grant" } }]);

    await auth.init();

    expect(auth.isAuthenticated()).toBe(false);
    expect(localStorage.getItem("olve-slimerepublics.oidc.refresh")).toBeNull();
  });
  ...
  it("login() stashes a PKCE verifier and redirects to the authorize endpoint with S256", async () => {
    ...
    const stash = JSON.parse(sessionStorage.getItem("olve-slimerepublics.oidc.pkce") ?? "{}");
