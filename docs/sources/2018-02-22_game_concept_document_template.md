...
*This is a skeleton/reference for a game design concept. Your full design documents will be much longer!* — the raw file's own subtitle, preserved verbatim.

**Source:** `2018-02-22_game_concept_document_template.raw.pdf`
**Date:** 2018-02-22
**Author:** Michael Sellers

> **Transformation notes.** This is a markdown rendering of the raw capture, not a summary.
> The raw is a single page laid out as a two-column table: a narrow left margin column holding
> three *rotated* group labels, and a wide right column holding fourteen prompt rows. The rotated
> labels are promoted to `##` headings here and the prompts to `###` headings; each prompt's
> guidance paragraph follows it verbatim.
> Because the group labels are rotated text spanning several rows, no reading order relates them
> to the prompts — the item→group assignment below was recovered from the PDF's x/y geometry
> (each label's vertical centre against the prompt rows it spans), giving groups of 5 / 5 / 4.
> Note the filename slug (`game_concept_document_template`) comes from the raw file's own name;
> the document's internal title is the one above.
> Typographic quotes are normalised to ASCII. No prose is otherwise altered.
